...
interface HostMetricRow {
  ...
  avg_cpu: number | null;
  max_cpu: number | null;
  avg_mem_used: number | null;
  max_mem_used: number | null;
...
interface DigestData {
  ...
  hostMetrics: DigestHostMetric[];
  hostCount: number;
  endpoints: DigestEndpoint[];
...
function buildDigest(db: Database.Database, config: DigestConfig): DigestData {
  ...
  // --- Host system metrics ---
  const hostMetrics: DigestHostMetric[] = (db.prepare(`
    SELECT host_id,
    ...
    WHERE collected_at BETWEEN ? AND ?
    GROUP BY host_id
  `).all(thisWeekStart, nowStr) as HostMetricRow[]).map(h => ({
    hostId: h.host_id,
  ...
  // --- HTTP endpoint stats ---
  let endpoints: DigestEndpoint[] = [];
  try {
    const { getEndpointsForDigest } = require('../http-monitor/queries');
  ...
  const triggeredAlertsThisWeek: DigestAlert[] = alertRows.map(r => ({
    type: r.alert_type,
    target: r.target,
    hostId: r.host_id,
    message: r.message,
  ...
  try {
    ...
    // rollup_anomalies table not yet present
  }

  // --- Host groups (from hosts table, using COALESCE override pattern) ---
