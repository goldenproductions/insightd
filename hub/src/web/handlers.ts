...
async function handleSetHostGroup(req: HandlerReq, res: ServerResponse, db: Database.Database, config: any, params: Record<string, string>): Promise<any> {
  ...
  const body = await readBody(req);
  const raw = body?.host_group;
  // null → manually ungrouped (empty string in DB). string → trimmed manual value.
  const value = raw == null ? '' : String(raw).trim();
  db.prepare('UPDATE hosts SET host_group_override = ? WHERE host_id = ?').run(value, hostId);
  return { hostId, host_group_override: value };
}
...
function handleContainerDetail(req: HandlerReq, res: ServerResponse, db: Database.Database, config: any, params: Record<string, string>, ctx: HandlerCtx): any {
  ...
  const image = queries.getContainerImage(db, params.hostId, params.containerName);
  // Cluster id (k8s only). Lets the UI build a topology link for this
...
function handleContainerPodEvents(req: HandlerReq, res: ServerResponse, db: Database.Database, config: any, params: Record<string, string>): any {
  const hostId = decodeURIComponent(params.hostId);
  const containerName = decodeURIComponent(params.containerName);
  return {
    host_id: hostId,
    container_name: containerName,
    events: queries.getPodEvents(db, hostId, containerName, 20),
  };
}
...
function handleHostK8sEvents(req: HandlerReq, res: ServerResponse, db: Database.Database, config: any, params: Record<string, string>): any {
  ...
  const sinceHours = sinceHoursParam ? parseInt(sinceHoursParam, 10) : undefined;
  const items = queries.getK8sEventsForHost(db, params.hostId, { limit, reason, namespace, sinceHours });
  const clusterId = queries.getClusterIdForHost(db, params.hostId);
...
async function handleDismissIngress(req: HandlerReq, res: ServerResponse, db: Database.Database, config: any, params: Record<string, string>): Promise<any> {
  ...
  const r = endpointQueries.dismissIngress(db, ingressId);
  if (!r.dismissed) {
    res.statusCode = 404;
...
async function handleUndismissIngress(req: HandlerReq, res: ServerResponse, db: Database.Database, config: any, params: Record<string, string>): Promise<any> {
  ...
  const r = endpointQueries.undismissIngress(db, ingressId);
  if (!r.undismissed) {
    res.statusCode = 404;
    return { error: 'Ingress not found' };
  }
...
function handleGetContainerRcaNeighbors(req: HandlerReq, res: ServerResponse, db: Database.Database, config: any, params: Record<string, string>): any {
  ...
  return {
    host_id: hostId,
    container_name: containerName,
    neighbors: queries.getRcaNeighbors(db, hostId, containerName, 10),
  };
}

const LOG_BURSTS_DEFAULT_WINDOW_MS = 60 * 60 * 1000; // 1 hour
const LOG_BURSTS_MAX_WINDOW_MS = 7 * 24 * 60 * 60 * 1000; // 7 days

function parseWindowMs(raw: string | undefined): number {
  if (!raw) return LOG_BURSTS_DEFAULT_WINDOW_MS;
  // Accept either "15m"/"6h"/"1d" or a raw ms number.
  const m = /^(\d+)\s*(s|m|h|d)?$/.exec(raw.trim());
  if (!m) return LOG_BURSTS_DEFAULT_WINDOW_MS;
  ...
  const ms = unit === 's' ? n * 1000
           ...
           : unit === 'd' ? n * 86_400_000
           : n;
  return Math.min(LOG_BURSTS_MAX_WINDOW_MS, Math.max(60_000, ms));
}

function handleGetContainerLogBursts(req: HandlerReq, res: ServerResponse, db: Database.Database, config: any, params: Record<string, string>): any {
  const hostId = decodeURIComponent(params.hostId);
  const containerName = decodeURIComponent(params.containerName);
  const url = new URL(req.url || '', 'http://localhost');
  const at = url.searchParams.get('at') || new Date().toISOString();
  const windowMs = parseWindowMs(url.searchParams.get('window') || undefined);
  return {
    host_id: hostId,
    container_name: containerName,
    at,
    window_ms: windowMs,
    bursts: queries.getLogBursts(db, hostId, containerName, at, windowMs),
  };
}
