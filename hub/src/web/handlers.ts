...
function handleContainersStorage(req: HandlerReq, res: ServerResponse, db: Database.Database): any {
  if (!requireAuth(req)) { res.statusCode = 401; return { error: 'Unauthorized' }; }
  const url = new URL(req.url, `http://${req.headers.host}`);
  const showInternal = url.searchParams.get('showInternal') === 'true';
  return queries.getContainersStorage(db, offlineThresholdMinutes(), showInternal);
}
