...
let prevStats: Record<string, NetStats> | null = null;
 ...
 */
function collectNetworkIO(config?: NetworkIOConfig): { rxBytesPerSec: number; txBytesPerSec: number } | null {
  ...
  try {
    ...
    const now = Date.now();
    const stats: Record<string, NetStats> = {};
