...
const { config } = require('./config') as { config: { allowUpdates: boolean } };
 ...
 */
async function performUpdate(docker: Dockerode, target: string, image: string): Promise<{ status: string; message: string }> {
  ...
  const createOpts: Record<string, any> = {
    name: oldName,
    Image: image,
    Env: ((info.Config.Env || []) as string[]).filter((e: string) => !e.startsWith('INSIGHTD_VERSION=')),
  ...
  if (target === 'agent') {
    ...
    try {
      ...
      logger.warn('updater', `Failed to remove old container: ${(err as Error).message}`);
    }
  }
 ...
 */
async function cleanupOldContainers(docker: Dockerode): Promise<void> {
  try {
    const containers = await docker.listContainers({ all: true });
    for (const c of containers) {
      ...
      if (name.endsWith('-old') && name.startsWith('insightd-')) {
        ...
        try {
          ...
          logger.warn('updater', `Failed to remove ${name}: ${(err as Error).message}`);
        }
      }
    }
