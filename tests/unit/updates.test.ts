...
describe('checkUpdates', () => {
  ...
  it('returns update check results', async () => {
    ...
    https.request = (url: any, opts: any, callback: any) => {
      ...
      if (typeof url === 'string' && url.includes('auth.docker.io')) {
        ...
        (res as any).statusCode = 200;
        (res as any).headers = { 'docker-content-digest': 'sha256:remotedigest' };
        process.nextTick(() => { callback(res); res.emit('end'); });
      }
      const req = new EventEmitter();
      (req as any).setTimeout = () => {};
      (req as any).end = () => {};
      (req as any).destroy = () => {};
      return req;
    };
  ...
  it('deduplicates by image', async () => {
    ...
    https.request = (url: any, opts: any, callback: any) => {
      const res = new EventEmitter();
      if (typeof url === 'string' && url.includes('auth.docker.io')) {
        (res as any).statusCode = 200; (res as any).headers = {};
        process.nextTick(() => { callback(res); res.emit('data', JSON.stringify({ token: 't' })); res.emit('end'); });
      } else {
        (res as any).statusCode = 200; (res as any).headers = { 'docker-content-digest': 'sha256:same' };
        process.nextTick(() => { callback(res); res.emit('end'); });
      }
      const req = new EventEmitter();
      (req as any).setTimeout = () => {}; (req as any).end = () => {}; (req as any).destroy = () => {};
      return req;
    };
